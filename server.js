...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //Retrait du ID du commentaire de la liste des id de commentaire de l'article
    ...
    //Retrait du ID du commentaire de la liste des id de commentaire de l'utilisateur
...
function loadDatabase() {
  // Load database, or throw exception on error
  try {
    const yamlDB = yaml.safeLoad(fs.readFileSync('db.yml', 'utf8'));
    database = yamlDB;
  } catch (e) {
    console.log(e);
  }
...
function saveDatabase() {
  // Save database, or throw exception on error
  try {
    const yamlDB = yaml.safeDump(database);

    fs.writeFile('db.yml', yamlDB, function (err) {
      if (err) throw err;
    });
  } catch (e) {
    console.log(e);
